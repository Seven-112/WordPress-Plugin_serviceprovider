...
/**
 * Plugin Name: service_provider
 * Plugin URI:https://www.service_provider.com/
 * Description:custom
 * Version: 1.0.0
 * Author: Bhumi
 * Author URI: https://www.service_provider.com/
 */
...
{
    add_menu_page("service_provider", "service_provider", "", "wp-service-provider-plugin", "next_wp_contact_form_call", "dashicons-admin-users");
    add_submenu_page("wp-service-provider-plugin", "Forms", "Forms", "manage_options", "wp-contact-form", "next_wp_contact_forms_call");
    ...
    if (isset($_FILES['csv_file'])) {
        ...
        if (move_uploaded_file($file['tmp_name'], $file_path)) {
            ...
            if (($handle = fopen($file_path, "r")) !== false) {
                ...
                fclose($handle);
                wp_redirect(admin_url().'edit.php?post_type=serviceprovider');
            }
        } else {
            // File upload error
            echo 'Error uploading file. Please try again.';
...
function create_posttype()
{
    ...
    register_post_type(
        ...
        array(
            'labels' => array(
                'name' => __('serviceprovider'),
                'singular_name' => __('serviceprovider')
            ),
            'public' => true,
            'has_archive' => true,
            'rewrite' => array('slug' => 'serviceprovider'),
            'show_in_rest' => true,
...
add_filter('manage_serviceprovider_posts_columns', 'add_custom_columns_to_post_type');



add_action('manage_serviceprovider_posts_custom_column', 'custom_serviceprovider_posts_column', 10, 2);
function custom_serviceprovider_posts_column($column, $post_id)
{
    switch ($column) {
